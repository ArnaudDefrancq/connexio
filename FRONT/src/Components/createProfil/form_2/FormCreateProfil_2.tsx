...
import { monthArray, REGEX_DATE_NAISSANCE, REGEX_TEXTE, REGEX_TEXTE_PROFIL } from '../../../Tools/config';
    ...
    if (e.target.files && e.target.files[0]) {      
      ...
      reader.onload = (event: ProgressEvent<FileReader>) => {
        ...
        if (profil) {
          ...
          setDataLocalStorage('formData', 'isClickProfil', true);
        } else {
          setImgBanner(filePreview); 
          setBannerFile(file); 
          setDataLocalStorage('formData', 'banner', event.target?.result as string); 
          setDataLocalStorage('formData', 'isClickBanner', true);
        }
      };
  ...
  useEffect(() => {
    if (localStorage.getItem('formData')) {
      ...
      if (storedData) {
        const parseData = JSON.parse(storedData);
        if (parseData.profil)  {
          if (idParams && !isEmpty(idParams) && !parseData.isClickProfil) {
        ...
        if (parseData.banner)  {
          if (idParams && !isEmpty(idParams) && !parseData.isClickBanner) {
      ...
      if (localStorage.getItem('formData')) {
        ...
        if (storedData) {
          ...
          if (idParams && !isEmpty(idParams)) {
            navigate(`/profil/${idParams}`);
            window.location.href = `/profil/${idParams}`;
          } else {
